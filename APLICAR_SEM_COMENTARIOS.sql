...
BEGIN
  ...
  RETURN EXISTS (
    ...
    WHERE ts.trilha_id = p_trilha_id AND ts.incluir = true
      AND (
        (ts.department_id IS NOT NULL AND ts.position_id IS NULL AND ts.department_id = v_colaborador.department_id)
        OR
        (ts.department_id IS NULL AND ts.position_id IS NOT NULL AND ts.position_id = v_colaborador.position_id)
        OR
