-- ============================================
-- SCRIPT SQL: Migração Dashboard Mock Data (CORRIGIDO)
-- Arquivo: supabase-migration-fixed.sql
-- Descrição: Script corrigido para executar diretamente no Supabase
-- ============================================
...
-- ============================================
-- 5. INSERIR DADOS MOCK (CORRIGIDO)
-- ============================================
...
('5978f911-738b-4aae-802a-f037fdac2e64', '4ab6c765-bcfc-4280-84cd-3190fcf881c2', 'Baixo engajamento em trilha', 'Colaborador demonstra baixo interesse nas trilhas de onboarding', 'alerta_baixo_engajamento', 'negativo', 'critica', 'ativo', 85, true, NOW()),
('5978f911-738b-4aae-802a-f037fdac2e64', '4ab6c765-bcfc-4280-84cd-3190fcf881c2', 'Múltiplas dúvidas não resolvidas', 'Colaborador tem várias dúvidas pendentes há mais de 3 dias', 'alerta_inatividade', 'neutro', 'alta', 'ativo', 70, true, NOW()),
('5978f911-738b-4aae-802a-f037fdac2e64', '4ab6c765-bcfc-4280-84cd-3190fcf881c2', 'Sentimento negativo crescente', 'Feedback negativo sobre processo de onboarding', 'alerta_sentimento_negativo', 'negativo', 'media', 'ativo', 60, true, NOW())
ON CONFLICT DO NOTHING;

-- Inserir melhorias/ações (CORRIGIDO - adicionando categoria obrigatória)
...
('5978f911-738b-4aae-802a-f037fdac2e64', '4ab6c765-bcfc-4280-84cd-3190fcf881c2', 'alerta_critico', '🚨 Alerta Crítico: Baixo Engajamento', 'Colaborador João Silva apresenta baixo engajamento nas trilhas', '{"alerta_id": "1", "severidade": "critica", "score": 85}', '/dashboard.html#alertas', NOW()),
('5978f911-738b-4aae-802a-f037fdac2e64', '4ab6c765-bcfc-4280-84cd-3190fcf881c2', 'acao_pendente', '⏳ Ação Pendente: Revisar Trilha', 'Aprovação necessária para ajustar trilha de desenvolvimento', '{"acao_id": "1", "tipo": "ajustar_trilha"}', '/dashboard.html#acoes', NOW())
ON CONFLICT DO NOTHING;

-- ============================================
-- 6. VERIFICAR DADOS INSERIDOS (CORRIGIDO)
-- ============================================
DO $$
DECLARE
...
BEGIN
    ...
    WHERE tenant_id = tenant_uuid AND alerta_gerado = true;
    ...
    FROM users 
    WHERE tenant_id = tenant_uuid AND risk_score > 0;
...
-- ============================================
-- 8. CRIAR VIEW OTIMIZADA PARA DASHBOARD (CORRIGIDO)
-- ============================================
...
SELECT 
    ...
    -- Métricas de trilhas (CORRIGIDO - removendo referência a status inexistente)
    (SELECT COUNT(*) FROM trilhas tr WHERE tr.tenant_id = t.id) as trilhas_ativas,
    (SELECT COUNT(*) FROM trilhas tr WHERE tr.tenant_id = t.id) as trilhas_concluidas,
...
-- ============================================
-- 9. CRIAR FUNÇÃO PARA BUSCAR DADOS DO DASHBOARD (CORRIGIDO)
-- ============================================
...
BEGIN
    ...
    SELECT * INTO dm_record FROM dashboard_metrics WHERE tenant_id = p_tenant_id;
    
    -- Se não encontrou dados, criar estrutura básica
    IF dm_record IS NULL THEN
        SELECT json_build_object(
            'tenant_id', p_tenant_id,
            'timestamp', NOW(),
            'trilhasAtivas', 0,
            'usuariosOnboarding', 0,
            'melhoriasSugeridas', 0,
            'sentimentoMedio', 0,
            'alertasAtivos', 0,
            'colaboradoresRisco', 0,
            'trilhasTrend', '+0%',
            'usuariosTrend', '+0%',
            'melhoriasTrend', '+0%',
            'sentimentoTrend', '+0%',
    ...
    ELSE
        ...
        SELECT json_build_object(
            ...
            -- Métricas principais
            'trilhasAtivas', COALESCE(dm_record.trilhas_ativas, 0),
            'usuariosOnboarding', COALESCE(dm_record.usuarios_onboarding, 0),
            'melhoriasSugeridas', COALESCE(dm_record.melhorias_sugeridas, 0),
            'sentimentoMedio', COALESCE(ROUND(dm_record.sentimento_medio::numeric, 1), 0),
            'alertasAtivos', COALESCE(dm_record.alertas_ativos, 0),
            'colaboradoresRisco', COALESCE(dm_record.colaboradores_alto_risco, 0),
            
            -- Tendências
            'trilhasTrend', CASE 
                WHEN dm_record.trilhas_concluidas > 0 THEN '+12%'
                ELSE '+0%'
            END,
            'usuariosTrend', CASE 
                WHEN dm_record.usuarios_onboarding > 0 THEN '+8%'
                ELSE '+0%'
            END,
            'melhoriasTrend', CASE 
                WHEN dm_record.melhorias_sugeridas > 0 THEN '+15%'
                ELSE '+0%'
            END,
            'sentimentoTrend', CASE 
                WHEN dm_record.sentimento_medio > 4 THEN '+0.3'
                WHEN dm_record.sentimento_medio < 3 THEN '-0.2'
                ELSE '+0.1'
            END,
            'alertasTrend', CASE 
                WHEN dm_record.alertas_criticos > 0 THEN '-25%'
                ELSE '+0%'
            END,
            'riscoTrend', CASE 
                WHEN dm_record.colaboradores_alto_risco > 0 THEN '-50%'
                ELSE '+0%'
            END,
            ...
            'graficos', json_build_object(
                ...
                'alertasPorSeveridade', json_build_array(
                    json_build_object('severidade', 'Crítico', 'quantidade', COALESCE(dm_record.alertas_criticos, 0)),
